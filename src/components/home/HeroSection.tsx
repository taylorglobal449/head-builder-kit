...
export function HeroSection({ products }: HeroSectionProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-4">
      {/* Left: Hero Banner + DeWalt Banner side by side */}
      <div className="grid grid-cols-[1fr_100px] lg:grid-cols-[1fr_180px] gap-4">
        {/* Main Hero Banner */}
        ...
        {/* DeWalt Vertical Banner - same row, matches hero height */}
        <Link 
          to="/brands/dewalt"
          className="relative overflow-hidden rounded-lg bg-gradient-to-b from-amber-400 to-amber-500 min-h-[200px] lg:min-h-[260px] flex flex-col items-center justify-center p-3 lg:p-4 hover:opacity-95 transition-opacity"
        >
          <span className="text-zinc-900 font-black text-sm lg:text-lg uppercase tracking-wide text-center mb-2">DEWALT</span>
          <span className="text-zinc-900 font-bold text-base lg:text-xl text-center leading-tight">UP TO</span>
          <span className="text-zinc-900 font-black text-2xl lg:text-4xl text-center leading-none">40% OFF</span>
          <span className="text-zinc-900/80 text-xs lg:text-sm text-center mt-2">Select Tools</span>
          <div className="mt-4 bg-zinc-900/20 rounded px-3 py-1.5">
            <span className="text-zinc-900 text-xs lg:text-sm font-bold">SHOP NOW →</span>
          </div>
        </Link>
      </div>
      ...
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1">
            <button 
              onClick={() => scroll('left')}
              className="w-7 h-7 rounded bg-muted hover:bg-muted/80 flex items-center justify-center transition-colors"
            >
            ...
            return (
              ...
              >
                <div className="relative aspect-square rounded-md overflow-hidden bg-muted mb-2">
                  ...
                  {hasDiscount && (
                    <div className="absolute top-1 left-1 bg-header-primary text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded">
                      {discountPercent}%
                    </div>
                  )}
                </div>
                <p className="text-xs font-medium text-foreground line-clamp-2 leading-tight group-hover:text-header-primary transition-colors">
                  {product.node.title}
                </p>
        ...
        <Link 
          to="/deals"
          className="block text-center text-xs font-bold text-header-primary hover:underline mt-3"
        >
          VIEW ALL DEALS →
        </Link>
      </div>
